...
# 1. LOAD YOUR DATA
# ==========================================
# Replace 'your_data-epo.fif' with the path to your actual epochs file.
...
for i, sub in enumerate(subjects):
    ...
    # Fit on the entire subject's dataset within the peak window
    ...
    # Extract the spatial patterns (returns shape: 36 OvO classifiers, n_channels)
    patterns = get_coef(pattern_clf, 'patterns_', inverse_transform=True)
    
    # Take the absolute mean across all 36 binary classifiers to get an overall "importance map"
    ...
    if RUN_TEMPORAL_GENERALIZATION:
        ...
        # Note: This trains a model at every timepoint and tests it on all other timepoints.
...
# ==========================================
# Average the accuracies across all 30 subjects
...
# Subtract chance level from scores so that the null hypothesis is a mean of 0
...
# Plot the Standard Error of the Mean (SEM) as a shaded region
...
plt.title('Time-Resolved Decoding of Spoken Digits (Group Level, N=' + str(n_subjects) + ')')
plt.xlabel('Time (s)')
plt.ylabel('Accuracy')
...
# ==========================================
# Average the raw confusion matrices (counts) across all subjects
group_conf_mx = np.mean(all_subject_conf_mx, axis=0)

# Normalize the group matrix by the true label (row-wise) to get proportions
with np.errstate(divide='ignore', invalid='ignore'):
    group_conf_mx_normalized = group_conf_mx / group_conf_mx.sum(axis=1, keepdims=True)
    group_conf_mx_normalized = np.nan_to_num(group_conf_mx_normalized)  # Handle cases with no trials for a class
...
# We use a sequential colormap (Reds) anchored at 0 because we took the absolute value of the patterns
...
if RUN_TEMPORAL_GENERALIZATION:
    ...
    # Perform cluster permutation test on the 2D TG matrix
    ...
    # Create a boolean mask of the significant 2D clusters
    sig_mask_tg = np.zeros(group_tg_scores.shape, dtype=bool)
    for c, p_val in zip(clusters_tg, cluster_p_values_tg):
        if p_val <= 0.05:
            sig_mask_tg |= c  # Combine all significant cluster masks

    fig, ax = plt.subplots(figsize=(7, 6))

    # Center the colormap perfectly at chance level (white = chance, red = above chance, blue = below chance)
    ...
    # Overlay significant clusters as black contours
    if sig_mask_tg.any():
