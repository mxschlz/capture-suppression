...
def run_tracking():
    ...
    video_src = 0  # or "/home/max/Projects/capture-suppression/obsolete/output.avi"
    ...
    face_detector = FaceDetector("/home/max/Projects/headpose/assets/face_detector.onnx")

    # Setup a mark detector to detect landmarks.
    mark_detector = MarkDetector("/home/max/Projects/headpose/assets/face_landmarks.onnx")
    ...
    while True:
        ...
        line_height = 30  # Adjust this based on your font size and desired spacing

        try:
            vertical_text = f"Vertical: {round(gaze.vertical_ratio(), 2)}"
            horizontal_text = f"Horizontal: {round(gaze.horizontal_ratio(), 2)}"
        except:
            vertical_text = f"Vertical: {gaze.vertical_ratio()}"
            horizontal_text = f"Horizontal: {gaze.horizontal_ratio()}"
        ...
        # Show preview.
        cv2.putText(new_frame, vertical_text, (60, 60),
                    cv2.FONT_HERSHEY_DUPLEX, 1.5, (255, 0, 0), 2)
        cv2.putText(new_frame, horizontal_text, (60, 60 + line_height),
                    cv2.FONT_HERSHEY_DUPLEX, 1.5, (255, 0, 0), 2)
