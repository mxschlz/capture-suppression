...
sub_ids = [103, 104, 105, 106, 108, 110, 112, 114]
# load epochs
...
diff_wave_distractor = contra_singleton_epochs_data.mean(axis=0) - ipsi_singleton_epochs_data.mean(axis=0)
# run ttests
...
# get the trial-wise data for targets
contra_target_epochs_data_all_chs = np.mean(np.concatenate([left_target_epochs.copy().get_data(),
                                                            right_target_epochs.copy().get_data()], axis=0),
                                            axis=0)
ipsi_target_epochs_data_all_chs = np.mean(np.concatenate([left_target_epochs.copy().get_data(),
                                                          right_target_epochs.copy().get_data()], axis=0),
                                          axis=0)
# get the trial-wise data for singletons
contra_singleton_epochs_data_all_chs = np.mean(np.concatenate([left_singleton_epochs.copy().get_data(),
                                                               right_singleton_epochs.copy().get_data()], axis=0),
                                               axis=0)
ipsi_singleton_epochs_data_all_chs = np.mean(np.concatenate([left_singleton_epochs.copy().get_data(),
                                                             right_singleton_epochs.copy().get_data()], axis=0),
                                             axis=0)
...
X = [contra_singleton_epochs_data, ipsi_singleton_epochs_data]
...
ax.plot(
    times,
    contra_singleton_epochs_data.mean(axis=0) - ipsi_singleton_epochs_data.mean(axis=0),
