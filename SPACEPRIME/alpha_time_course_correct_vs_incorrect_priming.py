...
epochs = mne.concatenate_epochs([mne.read_epochs(glob.glob(f"{get_data_path()}derivatives/epoching/sub-{subject}/eeg/sub-{subject}_task-spaceprime-epo.fif")[0], preload=True).pick(left_roi+right_roi) for subject in subject_ids])
...
alpha_power = epochs.compute_tfr(method=method, freqs=alpha_freqs, n_cycles=n_cycles,
                           decim=decim, n_jobs=n_jobs, return_itc=False, average=False)
