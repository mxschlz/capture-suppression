...
for i, trial in merged_diff_wave_df.iterrows():
    ...
    if "Target-1" in cond and "Singleton-2" in cond:
        ...
        df.loc[df.index[i], "N2ac_mean_amplitude"] = trial.FC6 - trial.FC5
        df.loc[df.index[i], "Pd_mean_amplitude"] = None
        df.loc[df.index[i], "Lateral"] = 1
    elif "Target-3" in cond and "Singleton-2" in cond:
        df.loc[df.index[i], "N2ac_mean_amplitude"] = trial.FC5 - trial.FC6
        df.loc[df.index[i], "Pd_mean_amplitude"] = None
        df.loc[df.index[i], "Lateral"] = 1
    # check for lateralized distractors and subtract contra - ipsi
    elif "Singleton-1" in cond and "Target-2" in cond:
        df.loc[df.index[i], "Pd_mean_amplitude"] = trial.C4 - trial.C3
        df.loc[df.index[i], "N2ac_mean_amplitude"] = None
        df.loc[df.index[i], "Lateral"] = 2
    elif "Singleton-3" in cond and "Target-2" in cond:
        df.loc[df.index[i], "Pd_mean_amplitude"] = trial.C3 - trial.C4
        df.loc[df.index[i], "N2ac_mean_amplitude"] = None
        df.loc[df.index[i], "Lateral"] = 2
    # insert None in case none of the above conditions are satisfied
    elif "Singleton-0" in cond:
        df.loc[df.index[i], "N2ac_mean_amplitude"] = None
        df.loc[df.index[i], "Pd_mean_amplitude"] = None
        df.loc[df.index[i], "Lateral"] = None
...
# some formatting
pd_stat_df = df[["Pd_mean_amplitude", "SingletonLoc", "subject_id"]].dropna(subset=["SingletonLoc"],
                                                                       ignore_index=True).astype(float)
model = smf.mixedlm(formula=formula, data=pd_stat_df, groups="subject_id")
result = model.fit()
result.summary()
sns.lmplot(data=pd_stat_df, x="SingletonLoc", y="Pd_mean_amplitude", hue="subject_id")
# Modeling N2ac
formula = "N2ac_mean_amplitude ~ TargetLoc"
# some formatting
n2ac_stat_df = df[["N2ac_mean_amplitude", "TargetLoc", "subject_id"]].dropna(subset=["N2ac_mean_amplitude", "TargetLoc"],
                                                                       ignore_index=True).astype(float)
model = smf.mixedlm(formula=formula, data=n2ac_stat_df, groups="subject_id")
result = model.fit()
result.summary()
