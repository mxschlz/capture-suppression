...
print(f"Starting cross-condition decoding for LOCATIONS (Train: Target (distractor-absent) -> Test: Targets/Controls/Distractors) for {n_subjects} subjects...")
...
def get_valid_mask(y):
    if not isinstance(y, np.ndarray):
        y = np.array(y)
    # We want to exclude missing, null, None, or '0' values which might indicate absent locations
    mask = []
    for val in y:
        if pd.isna(val):
            mask.append(False)
        elif str(val).lower() in ['none', 'nan', 'null', '0', '0.0', '']:
            mask.append(False)
        else:
            mask.append(True)
    return np.array(mask)

# To figure out the number of classes dynamically
...
for i, sub in enumerate(subjects):
    ...
    train_mask = (singleton_present == 0) & get_valid_mask(y_target)
    X_train = epochs.get_data()[train_mask]
    y_train = y_target[train_mask]

    # --- Test Data (Distractor-Present) ---
    test_mask = (singleton_present == 1) & get_valid_mask(y_target) & \
                get_valid_mask(y_distractor) & get_valid_mask(y_control)
    ...
    if len(y_train) > 0:
        all_unique_classes.update(np.unique(y_train))

    if len(y_train) > 0 and len(y_test_target) > 0:
        print(f"  Training on Target Locations (distractor-absent, N={len(y_train)}), Testing on Targets/Controls/Distractors (distractor-present, N={len(y_test_target)})...")
...
# Determine chance level dynamically
n_classes = len(all_unique_classes) if len(all_unique_classes) > 0 else 3
if n_classes == 0:
    n_classes = 3 # fallback
print(f"Detected {n_classes} unique target locations: {all_unique_classes}")
...
if run_temporal_generalization:
    ...
    im1 = plot_tg(ax, mean_tg_distractor, sig_mask_tg_distractor, f'TG: Train on Target, Test on Distractor Locs\n(N={n_subjects})')
