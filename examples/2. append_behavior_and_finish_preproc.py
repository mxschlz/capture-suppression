...
def append_behavior(subject, epochs, bad_subjects=None):
    if subject_id not in bad_subjects:
        ...
        if not bad_subjects:
            ...
            while True:
                try:
                    event_codes[visual_event_indices] = 20
                    event_codes[auditory_event_indices] = 80
                    print(f"No mismatch between epochs length and visual/auditory events in subject {subject_id}. \n"
                    ...
                    print(f"Found mismatch between epochs length and visual/auditory events in subject {subject_id}. \n"
        ...
        print(f"Cannot process subject {subject_id}")
        pass
    return epochs
...
for subject_id in bad_subjects[0:1]:
    ...
    epochs_appended = append_behavior(subject=subject, epochs=epochs, bad_subjects=very_bads)
    # cut epochs
    flat = dict(eeg=1e-6)
    reject = dict(eeg=250e-6)
    # if subject_id in ["SP_EEG_P0054", 'SP_EEG_P0085']:
