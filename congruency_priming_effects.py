...
no_priming_trials = df[(df['congruency_priming'] == 'no_priming') & (df['Singletonpres'] == 1)]
positive_priming_trials = df[(df['congruency_priming'] == 'positive_priming') & (df['Singletonpres'] == 1)]
negative_priming_trials = df[(df['congruency_priming'] == 'negative_priming') & (df['Singletonpres'] == 1)]
...
df_mean = df.groupby(['subject', 'congruency_priming']).mean(numeric_only=True).reset_index()

# make barplot
barplot = sns.barplot(data=df, y=df.correct, x=df.congruency_priming)
...
for subject in subjects:
    ...
    x_positions = [bar_positions[i] for i, _ in enumerate(subject_data['congruency_priming'])]
    plt.plot(x_positions, subject_data['correct'], marker='o', linestyle='-', color='grey', alpha=0.5)

plt.savefig("/home/max/figures/congruency_priming_correct.png", dpi=400)
plt.close()
...
# make barplot
barplot = sns.barplot(data=df, y=df.RT, x=df.congruency_priming)
...
plt.savefig("/home/max/figures/congruency_priming_rt.png", dpi=400)
plt.close()
...
anova_correct = AnovaRM(df, depvar='correct', subject='subject', within=['congruency_priming'], aggregate_func="mean").fit()
print(anova_correct.summary())

# ad hoc stats
# Example for the 'correct' variable
pvals = []
for priming1 in df['congruency_priming'].unique():
    for priming2 in df['congruency_priming'].unique():
        if priming1 != priming2:
            subset = df[(df['congruency_priming'] == priming1) | (df['congruency_priming'] == priming2)]
            print(f"Comparison of: {priming1, priming2} ")
            t_stat, p_val = stats.ttest_ind(subset[subset['congruency_priming'] == priming1]['correct'],
                                            subset[subset['congruency_priming'] == priming2]['correct'])
            pvals.append(p_val)
