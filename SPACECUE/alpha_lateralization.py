...
for subject in subject_ids[:]:
    ...
    try:
        epoch_file_pattern = os.path.join(SPACECUE.get_data_path(), "derivatives", "epoching", f"sci-{subject}", "eeg",
                                          f"sci-{subject}_task-SPACECUE-epo.fif")
    ...
    try:
        ...
        # Extract based on expected distractor location
        if epochs_sub.metadata is not None and 'HP_Distractor_Loc' in epochs_sub.metadata.columns:
            # To robustly handle potential NaNs or mixed types in 'HP_Distractor_Loc',
            # we first coerce the column to a numeric type. This turns non-numeric
            # values into NaNs, which then correctly evaluate to False in comparisons.
            hp_loc_numeric = pd.to_numeric(epochs_sub.metadata['HP_Distractor_Loc'], errors='coerce')
            left_mask = hp_loc_numeric == 1.0
            right_mask = hp_loc_numeric == 3.0
            left_expected_epochs_sub = epochs_sub[left_mask].copy()
            right_expected_epochs_sub = epochs_sub[right_mask].copy()
            print(f"Expected left trials: {len(left_expected_epochs_sub)}, \n"
                  f"Expected right trials: {len(right_expected_epochs_sub)}")
        else:
            left_expected_epochs_sub, right_expected_epochs_sub = None, None
    except Exception as e:
    ...
    try:
        ...
        if left_expected_epochs_sub is not None and len(left_expected_epochs_sub) > 0 and len(right_expected_epochs_sub) > 0:
            alpha_diff_expected_lr, _ = get_lateralization_index(
                left_expected_epochs_sub, right_expected_epochs_sub, picks_for_diff,
                freqs, n_cycles, decim, method, tmin_analysis, tmax_analysis, n_jobs_tfr=n_jobs
            )
            subject_results['expected_li'].append(alpha_diff_expected_lr)
...
# Align time vectors across all subjects/conditions if they differ due to cropping bounds
all_tfrs = subject_results['target_li'] + subject_results['singleton_li'] + subject_results['expected_li']
time_lengths = [len(tfr.times) for tfr in all_tfrs]

if len(set(time_lengths)) > 1:
    print(f"\nWarning: Time lengths vary across subjects (e.g., {min(time_lengths)} to {max(time_lengths)}). Aligning to common time window...")
    common_tmin = max(tfr.times[0] for tfr in all_tfrs)
    common_tmax = min(tfr.times[-1] for tfr in all_tfrs)

    for tfr in all_tfrs:
        tfr.crop(tmin=common_tmin, tmax=common_tmax)

    # Force exact shape match in case of float/decimation discrepancies
    shortest_tfr = min(all_tfrs, key=lambda x: len(x.times))
    times_vector = shortest_tfr.times.copy()
    from scipy.interpolate import interp1d
    from mne.time_frequency import AverageTFRArray

    for i, tfr in enumerate(all_tfrs):
        if len(tfr.times) != len(times_vector) or not np.allclose(tfr.times, times_vector):
            f_interp = interp1d(tfr.times, tfr.data, axis=-1, bounds_error=False, fill_value="extrapolate")
            new_data = f_interp(times_vector)
            all_tfrs[i] = AverageTFRArray(
                info=tfr.info, data=new_data, times=times_vector, freqs=tfr.freqs,
                nave=tfr.nave, comment=tfr.comment, method=tfr.method
            )

    # Update the original lists
    len_t = len(subject_results['target_li'])
    len_s = len(subject_results['singleton_li'])
    subject_results['target_li'] = all_tfrs[:len_t]
    subject_results['singleton_li'] = all_tfrs[len_t:len_t+len_s]
    subject_results['expected_li'] = all_tfrs[len_t+len_s:]
else:
    times_vector = all_tfrs[0].times.copy()
...
fig_tfr, axes_tfr = plt.subplots(1, 3, figsize=(21, 6), sharey=True)
axes_tfr = axes_tfr.flatten()

ga_li_target.plot(axes=axes_tfr[0], show=False, colorbar=True, combine="mean", cmap="coolwarm")
...
ga_li_singleton.plot(axes=axes_tfr[1], show=False, colorbar=True, combine="mean", cmap="coolwarm")
...
ga_diff.plot(axes=axes_tfr[2], show=False, colorbar=True, combine="mean", cmap="coolwarm")
axes_tfr[2].axhline(0, color='k', linestyle='--', linewidth=0.5)
axes_tfr[2].axvline(0, color='k', linestyle=':', linewidth=0.5)
axes_tfr[2].set_title(f"Difference (Target - Singleton)\n(N={len(processed_subjects)})")
axes_tfr[2].set_xlabel("Time (s)")
sns.despine(ax=axes_tfr[2])
...
# --- Expected Distractor Analysis (Appended as requested) ---
print("\n--- Processing Expected Distractor Analysis ---")
if len(subject_results['expected_li']) > 0:
    ga_li_expected = mne.grand_average(subject_results['expected_li'])

    # Plot Grand Average LI TFR for Expected Distractor
    fig_tfr_exp, ax_tfr_exp = plt.subplots(1, 1, figsize=(8, 6))
    ga_li_expected.plot(axes=ax_tfr_exp, show=False, colorbar=True, combine="mean", cmap="coolwarm")
    ax_tfr_exp.axhline(0, color='k', linestyle='--', linewidth=0.5)
    ax_tfr_exp.axvline(0, color='k', linestyle=':', linewidth=0.5)
    ax_tfr_exp.set_title(f"GA Expected Distractor LI ({freqs[0]}-{freqs[-1]} Hz)\n(N={len(subject_results['expected_li'])})")
    ax_tfr_exp.set_xlabel("Time (s)")
    ax_tfr_exp.set_ylabel("Frequency (Hz)")
    sns.despine(ax=ax_tfr_exp)
    plt.tight_layout()

    # Hemispheric Time Courses for Expected Distractor
    expected_data_time_courses = dict(left_hemisphere=[], right_hemisphere=[])
    
    for i, li_sub in enumerate(subject_results['expected_li']):
        ch_names_sub_e = li_sub.ch_names
        valid_left_roi_e = [ch for ch in left_roi_hemi if ch in ch_names_sub_e]
        valid_right_roi_e = [ch for ch in right_roi_hemi if ch in ch_names_sub_e]
        
        if not valid_left_roi_e or not valid_right_roi_e:
            continue
        
        tfr_exp_left = li_sub.copy().pick(valid_left_roi_e).crop(fmin=alpha_fmin_hemi, fmax=alpha_fmax_hemi).data.mean(axis=(0, 1))
        tfr_exp_right = li_sub.copy().pick(valid_right_roi_e).crop(fmin=alpha_fmin_hemi, fmax=alpha_fmax_hemi).data.mean(axis=(0, 1))
        expected_data_time_courses["left_hemisphere"].append(tfr_exp_left)
        expected_data_time_courses["right_hemisphere"].append(tfr_exp_right)
    
    expected_left_tc_all = np.array(expected_data_time_courses["left_hemisphere"]) if expected_data_time_courses["left_hemisphere"] else np.empty((0, len(times_vector)))
    expected_right_tc_all = np.array(expected_data_time_courses["right_hemisphere"]) if expected_data_time_courses["right_hemisphere"] else np.empty((0, len(times_vector)))
    
    n_subs_expected_tc = expected_left_tc_all.shape[0]
    
    # Cluster Permutation Test for Expected Distractor
    t_obs_e, clusters_e, cluster_pv_e, t_thresh_cluster_expected = None, None, None, None
    if n_subs_expected_tc >= 2:
        diff_expected_tc = expected_left_tc_all - expected_right_tc_all
        df_e = n_subs_expected_tc - 1
        t_thresh_cluster_expected = t.ppf(1 - alpha_stat_cluster / 2, df_e)
        print(f"\n--- Running Cluster Permutation Test for Expected Hemispheric LI (N={n_subs_expected_tc}) ---")
        t_obs_e, clusters_e, cluster_pv_e, _ = permutation_cluster_1samp_test(
            diff_expected_tc, threshold=t_thresh_cluster_expected, n_permutations=n_permutations_cluster,
            tail=cluster_tail, n_jobs=n_jobs, out_type="mask", seed=42, verbose=False
        )
    
    # Plot Hemispheric Time Courses and Stats for Expected Distractor
    ga_expected_left = expected_left_tc_all.mean(axis=0) if n_subs_expected_tc > 0 else np.full_like(times_vector, np.nan)
    sem_expected_left = sem(expected_left_tc_all, axis=0, nan_policy='omit') if n_subs_expected_tc > 0 else np.full_like(times_vector, np.nan)
    ga_expected_right = expected_right_tc_all.mean(axis=0) if n_subs_expected_tc > 0 else np.full_like(times_vector, np.nan)
    sem_expected_right = sem(expected_right_tc_all, axis=0, nan_policy='omit') if n_subs_expected_tc > 0 else np.full_like(times_vector, np.nan)
    
    fig_hemi_exp, axes_hemi_exp = plt.subplots(2, 1, figsize=(8, 10), sharex=True)
    fig_hemi_exp.suptitle("Expected Distractor: Hemispheric Alpha LI Time Courses", fontsize=16)
    
    ax = axes_hemi_exp[0]
    ax.plot(times_vector, ga_expected_left, label="Left Hemisphere LI", color="blue")
    ax.fill_between(times_vector, ga_expected_left - sem_expected_left, ga_expected_left + sem_expected_left, color="blue", alpha=0.2)
    ax.plot(times_vector, ga_expected_right, label="Right Hemisphere LI", color="red")
    ax.fill_between(times_vector, ga_expected_right - sem_expected_right, ga_expected_right + sem_expected_right, color="red", alpha=0.2)
    ax.axhline(0, color='k', linestyle=':', linewidth=0.8)
    ax.axvline(0, color='k', linestyle=':', linewidth=0.8)
    ax.set_ylabel(f"Alpha LI ({alpha_fmin_hemi}-{alpha_fmax_hemi} Hz)")
    ax.set_title(f"Expected distractors (N={n_subs_expected_tc})")
    ax.legend(loc="best")
    sns.despine(ax=ax)
    
    ax = axes_hemi_exp[1]
    if t_obs_e is not None:
        ax.plot(times_vector, t_obs_e, label="Observed T-value", color="black", linewidth=1.5)
        if t_thresh_cluster_expected is not None:
            ax.axhline(t_thresh_cluster_expected, color='gray', linestyle='--', linewidth=1, label=f'Cluster threshold (t={t_thresh_cluster_expected:.2f})')
            if cluster_tail == 0:
                ax.axhline(-t_thresh_cluster_expected, color='gray', linestyle='--', linewidth=1)
        
        significant_cluster_found_e = False
        if clusters_e is not None:
            for i, cl_mask in enumerate(clusters_e):
                if cluster_pv_e[i] < alpha_stat_cluster:
                    cluster_times = times_vector[cl_mask]
                    if len(cluster_times) > 0:
                        label_str_sig = f'Significant Cluster (p < {alpha_stat_cluster})' if not significant_cluster_found_e else '_nolegend_'
                        significant_cluster_found_e = True
                        ax.hlines(y=SIG_LINE_Y_OFFSET_TVAL, xmin=cluster_times[0], xmax=cluster_times[-1],
                                  color=SIG_LINE_COLOR_TVAL, linewidth=SIG_LINE_LW_TVAL, alpha=SIG_LINE_ALPHA_TVAL,
                                  label=label_str_sig)
        if significant_cluster_found_e or (t_thresh_cluster_expected is not None):
            ax.legend(loc="best")
    else:
        ax.text(0.5, 0.5, "No data/stats", ha='center', va='center', transform=ax.transAxes)
    
    ax.axhline(0, color='k', linestyle=':', linewidth=0.8)
    ax.axvline(0, color='k', linestyle=':', linewidth=0.8)
    ax.set_xlabel("Time (s)")
    ax.set_ylabel("T-value")
    ax.set_title("Expected Distractor: Cluster Test (L-R Hemi LI)")
    sns.despine(ax=ax)
