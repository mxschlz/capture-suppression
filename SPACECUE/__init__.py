...
def get_data_path():
    ...
    """
    if os.name == 'nt':
        data_path = 'G:\\Meine Ablage\\PhD\\data\\SPACECUE\\'
    elif os.name == 'posix':
        hostname = socket.gethostname()
        if "rechenknecht" in hostname:
            data_path = '/home/maxschulz/IPSY1-Storage/Projects/ac/Experiments/running_studies/SPACECUE/'
        elif 'MaxPC' in hostname:
            data_path = '/home/max/Insync/schulz.max5@gmail.com/GoogleDrive/PhD/data/SPACECUE/'
        else:
...
def _create_and_save_concatenated_epochs(paradigm, epochs_output_path):
    ...
    for subject in subject_ids:
        ...
        try:
            ...
            epoch_file_pattern = os.path.join(get_data_path(), "derivatives", "epoching", f"sci-{subject}", "eeg",
                                              f"sci-{subject}_task-{paradigm}-epo.fif")
...
print("INFO:   epochs = SPACECUE.load_concatenated_epochs('passive')")
print("INFO:   my_dataframe = SPACECUE.load_concatenated_csv('your_file.csv')")

print("\nINFO: To create missing files for a paradigm, use:")
print("INFO:   SPACECUE.concatenate_eeg_and_save('flanker')")
