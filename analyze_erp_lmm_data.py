...
n2ac_path = f'{SPACEPRIME.get_data_path()}concatenated\\{paradigm}_n2ac_erp_behavioral_lmm_long_data_between-within.csv'
pd_path = f'{SPACEPRIME.get_data_path()}concatenated\\{paradigm}_pd_erp_behavioral_lmm_long_data_between-within.csv'
...
if 'st_latency_50' in n2ac_df.columns:
    ...
    try:
        model = smf.mixedlm("target_towardness ~ Priming + st_latency_50 + st_mean_amp_50 + total_trial_nr + st_mean_amp_50:st_latency_50", data=n2ac_df, groups=n2ac_df["subject_id"])
        result = model.fit()
...
if 'st_latency_50' in pd_df.columns:
    ...
    try:
        model = smf.mixedlm("target_towardness ~ Priming + st_latency_50 + st_mean_amp_50 + total_trial_nr + st_mean_amp_50:st_latency_50", data=pd_df, groups=pd_df["subject_id"])
        result = model.fit()
        print(result.summary())
